...
function adminResetCounters() {
  const confirm = window.confirm("Tem certeza que deseja reiniciar todos os contadores? Esta ação não pode ser desfeita.");
...
function clearClickDataDirect() {
  ...
  if (confirm("Dados limpos! Deseja recarregar a página para ver as mudanças?")) {
...
function exportClickData() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(JSON.stringify(exportData, null, 2))
      ...
      .catch(() => console.log("Não foi possível copiar para a área de transferência"));
  }
...
function importClickData(data) {
  try {
    ...
    if (data.features) {
      localStorage.setItem("conectatea-feature-clicks", JSON.stringify(data.features));
    }
    
    if (data.pricing) {
      localStorage.setItem("conectatea-pricing-clicks", JSON.stringify(data.pricing));
    }
